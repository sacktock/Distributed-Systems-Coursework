...
class RequestHandler(object):
    ...
    def get_menu(self):
        ...
        try:
            menu = pd.read_csv(os.path.join('replica-server-1-resources/menu.csv')) # read the menu.csv file into a dataframe object
        except:
    ...
    def make_order(self, user_code, item, price, date_time, post_code, propagate_bit):
        ...
        try:
            f = open(os.path.join('replica-server-1-resources/orders.csv'), 'a') # open the orders.csv file
    ...
    def get_orders(self, user_code):
        ...
        try:
            orders = pd.read_csv(os.path.join('replica-server-1-resources/orders.csv')) # read the orders.csv file into a dataframe object
        except:
            print('sending error back ...') # send error back if file can't be read
